...
const Header: React.FC<HeaderProps> = ({
	...
	// Список /cases — тёмная шапка; детальная /cases/[slug] — светлая.
	...
	const logoMobileSrc = isLight ? '/img/logo-mobile-black.svg' : '/img/logo-mobile.svg';
	...
	return (
		<>
			...
			<div
				className={`fixed top-0 left-0 right-0 z-[70] transition-transform duration-300 ${showSticky && !isAtTop ? 'translate-y-0' : '-translate-y-full'
					}`}
				...
				<div className="container mx-auto px-4 sm:px-6 lg:px-8">
					...
					<div className="flex items-center justify-between h-28">
						...
						<div className="flex items-center gap-0">
							<button
								className={`logo-burger p-2 gap-2.5 px-4 py-2.5 ${isLight ? 'text-[#212121] bg-[#F1F2F4]' : 'text-white bg-[#333]'
							...
							<ul className="flex flex-row items-center gap-3 w-full">
								...
								<li className="relative">
									<button
										...
										className={`${navLinkBase} ${navText} ${navHover} ${isServicesActive ? activeBox : ''
								...
								<li className="relative">
									<button
										...
										className={`${navLinkBase} ${navText} ${navHover} ${isCompanyActive ? activeBox : ''
											} flex items-center gap-1 dropdown-toggle focus:outline-none`}
									>
										Компания
										<svg
							...
							<ul className="flex flex-col gap-2">
								...
								<li>
									<button
										...
										className={`w-full text-left text-[#212121] hover:bg-black/5 px-3 py-3 rounded-xl flex items-center justify-between ${isServicesActive ? 'outline outline-1 outline-[#E3E8F2]' : ''
											}`}
									>
										<span>Услуги</span>
								...
								<li>
									<button
										...
										className={`w-full text-left text-[#212121] hover:bg-black/5 px-3 py-3 rounded-xl flex items-center justify-between ${isCompanyActive ? 'outline outline-1 outline-[#E3E8F2]' : ''
											}`}
									>
										<span>Компания</span>
