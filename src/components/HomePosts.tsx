        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 mb-2">
          {posts.map((p) => (
            <Link
              key={p.id}
              href={`/blog/${p.slug}`}
              className="group bg-white rounded-3xl p-2.5 flex flex-col h-full transition-all duration-300 border border-[#fff] hover:border-[#0077FF]"
            >
              ...
              <div className="px-4 pb-4 flex flex-col flex-grow">
                ...
                <span className="inline-flex items-center justify-center self-start px-6 py-3 bg-[#F6F7F9] text-[#313131] text-lg font-medium rounded-xl border border-transparent group-hover:outline-1 group-hover:outline-[#0077FF]">
                  Подробнее
                </span>
              </div>
            </Link>
