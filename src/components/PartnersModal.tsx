...
const PartnersModal: React.FC<PartnersModalProps> = ({ open, onClose }) => {
  const [showConfirm, setShowConfirm] = useState(false);

  useEffect(() => {
    if (!open) return;
    const formContainer = document.getElementById("b24-partners-form");
    if (formContainer) formContainer.innerHTML = "";
    const script = document.createElement("script");
    // скрипт оставляем таким же — позже вы замените параметры
          ...
          <div className="p-4 max-w-xs w-full flex flex-col items-center">
            ...
            <div className="flex gap-4 mt-2">
              ...
              <button
                ...
                className="px-6 py-2 rounded-xl bg-gray-200 text-black font-medium hover:bg-gray-300"
              >
                Отмена
              </button>
            </div>
