								...
								return (
									<Link key={it.id} href={`/cases2/${it.slug}`} className={`group ${isWide ? 'md:col-span-2 lg:col-span-2 xl:col-span-2' : ''}`}>
										<div className="bg-white rounded-2xl p-2 flex h-full transition-all duration-300 border border-[#E5E7EB] hover:border-[#029cda]">
											<div className={`bg-[#F6F7F9] rounded-xl p-4 pl-3 flex flex-col ${isWide ? 'md:flex-row' : ''} h-full items-stretch relative overflow-hidden min-h-[420px] ${isWide ? 'md:min-h-[360px]' : ''}`}>
												<div className={`w-full mb-4 ${isWide ? 'md:w-1/2 md:flex md:justify-start md:items-start md:ml-0 md:order-2' : ''}`}>
													<div className={`relative w-full max-w-[320px] aspect-[4/3] rounded-xl flex items-start justify-start overflow-hidden ${isWide ? 'md:w-full md:max-w-none' : ''}`}>
														<Image src={it.cover || '/img/cases/case1.png'} alt={it.title} fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" />
													</div>
												</div>
												<div className={`flex flex-col justify-between flex-1 ${isWide ? 'md:w-1/2 md:order-1' : ''}`}>
													<div>
														...
														<h3 className={`text-xl font-medium text-[#313131] leading-tight mb-3 ${isWide ? 'md:text-2xl' : ''}`}>{it.title}</h3>
														{it.subtitle ? (
															<p className={`text-sm text-[#7C8A9A] mb-4 line-clamp-2 ${isWide ? 'md:text-base md:line-clamp-3' : ''}`}>{it.subtitle}</p>
														) : null}
														{it.location ? (
															<div className="flex items-center gap-1 mb-3">
																<svg className="w-4 h-4 text-[#029cda] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
																	<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
																</svg>
																<span className="text-sm text-[#7C8A9A]">{it.location}</span>
															</div>
														) : null}
													</div>
				...
				<section className="bg-[#F6F7F9] py-16">
					<div className="max-w-[1480px] mx-auto px-5 md:px-8">
						<div className="max-w-4xl mx-auto text-center">
							...
							<button onClick={openConsult} className="bg-[#029cda] text-white px-8 py-4 rounded-xl font-bold text-lg hover:bg-[#029cda]/90 transition-colors">Получить консультацию</button>
						</div>
					</div>
				</section>
			</div>
