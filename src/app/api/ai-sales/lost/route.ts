...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await getLostDealAnalytics(managerFilterFor(user), { from: sp.get("from"), to: sp.get("to") });
    return NextResponse.json(data);
  } catch (e) {
    const message = e instanceof Error ? e.message : "Ошибка аналитики проигрышей";
