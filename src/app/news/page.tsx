            ...
            <div className="w-full max-w-[761px]">
              <section className="bg-white rounded-3xl p-6 border">
                <div className="flex items-center justify-between">
                  ...
                  <Link href="/blog/index.html" className="h-9 px-3 rounded-lg border text-sm flex items-center">К статьям</Link>
                </div>
                <div className="mt-5 grid grid-cols-1 gap-4">
                  {news.map(n => (
                    <Link key={n.id} href={`/n?s=${encodeURIComponent(n.slug)}`} className="rounded-2xl border p-4 hover:border-[#2777ff] bg-white">
