...
export default function NewsPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#f2f3f7]" />}>
      <NewsPageInner />
    </Suspense>
  );
}
...
function NewsList() {
  ...
  // вкладки для LeftNav — как на странице блога
  ...
  // Обработка кликов по LeftNav — возвращаемся в блог
              ...
              <section className="bg-white rounded-3xl p-6 border">
                ...
                <div className="mt-5 grid grid-cols-1 gap-4">
                  {news.map((n) => (
                    <Link
                      key={n.id}
                      href={`/news?s=${encodeURIComponent(n.slug)}`}
